...
public class MenuListner implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        } else if (menu.getPluginItemList().contains((JMenuItem) e.getSource())) {
            String name = ((JMenuItem) e.getSource()).getText();
            boolean config = name.contains(m.mes.getString("Menu.18"));

            if (config && name.substring(0, 9).equals("Configure")) {
                name = name.substring(10);
            } else if (config) {
                name = name.substring(13);
            }

            PluginForJmjrst plugin = null;
            for (PluginForJmjrst pluginInList : menu.getPluginList()) {
                if (pluginInList.getName().equals(name)) {
                    plugin = pluginInList;
                    break;
                }
            }

            if (plugin != null && config) {
                plugin.configure();
            } else if (plugin != null) {
                plugin.run();
            } else {
                JLabel message = new JLabel();
                message.setText(m.mes.getString("Messages.3"));
                JOptionPane.showMessageDialog(m, message, m.mes.getString("Menu.17"),
