...
public class Menu extends JMenuBar {
    ...
    private final List<JMenuItem> pluginItemList;
    private final List<PluginForJmjrst> pluginList;
    ...
    public Menu(Main m) {
        ...
        JMenu pluginMenu = new JMenu(m.mes.getString("Menu.17"));
        ...
        pluginList = (ArrayList<PluginForJmjrst>) PluginManagement.getPlugins();
        pluginItemList = new ArrayList<>();
        for (PluginForJmjrst plugin : pluginList) {
            plugin.init(m);
            String pluginName = plugin.getName();
            JMenuItem pluginItem = new JMenuItem(pluginName);
            pluginItemList.add(pluginItem);
            pluginMenu.add(pluginItem);

            if (plugin.isConfigurable()) {
                JMenuItem config = new JMenuItem(m.mes.getString("Menu.18") + " " + pluginName);
                pluginItemList.add(config);
                pluginMenu.add(config);
            }
            pluginMenu.addSeparator();
        }

        //Show no plugins message or remove last separator
        if (pluginItemList.size() == 0) {
            JMenuItem message = new JMenuItem(m.mes.getString("Menu.19"));
            message.setEnabled(false);
            pluginMenu.add(message);
        } else {
            pluginMenu.remove(pluginMenu.getItemCount() - 1);
        }
        ...
        //Add action listener to all plugin items
        for (JMenuItem item : pluginItemList) {
    ...
    public List<JMenuItem> getPluginItemList() {
        return pluginItemList;
    }

    public List<PluginForJmjrst> getPluginList() {
        return pluginList;
    }
}
